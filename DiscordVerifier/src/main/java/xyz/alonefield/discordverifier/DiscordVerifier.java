...
public final class DiscordVerifier extends JavaPlugin {
    ...
    private void setupDatabase() {
        getServer().getScheduler().runTaskAsynchronously(this, () -> {
            try {
                getDatabaseConnection().prepareStatement("CREATE TABLE IF NOT EXISTS discord_verifier (uuid VARCHAR(36) NOT NULL, discord_id VARCHAR(36) NOT NULL, PRIMARY KEY (uuid))").execute();
            }
            catch (Exception e) {
                getLogger().severe("Failed to create database table");
                e.printStackTrace();
            }
        });
    }
